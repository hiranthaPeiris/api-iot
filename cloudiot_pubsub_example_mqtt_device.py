...
This example represents a simple device with a meterReading sensor and a fan
(simulated with software). When the device's fan is turned on, its meterReading
decreases by one degree per second, and when the device's fan is turned off,
its meterReading increases by one degree per second.

Every second, the device publishes its meterReading reading to Google Cloud IoT
Core. The server meanwhile receives these meterReading readings, and decides
whether to re-configure the device to turn its fan on or off. The server will
instruct the device to turn the fan on when the device's meterReading exceeds 10
degrees, and to turn it off when the device's meterReading is less than 0
...
class Device(object):
    ...
    def update_sensor_data(self):
        ...
        If the fan is on, assume the meterReading decreased one degree,
...
def main():
    ...
    # Update and publish meterReading readings at a rate of one per second.
    for _ in range(args.num_messages):
        # In an actual device, this would read the device's sensors. Here,
        # you update the meterReading based on whether the fan is on.
        device.run_meter()
        device.update_sensor_data()

        # Report the device's meterReading to the server by serializing it
        # as a JSON string.
        payload = json.dumps({'meterReading': device.meterReading})
